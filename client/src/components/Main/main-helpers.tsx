...
export function fetchArtistsWithOffset(
  code: string,
  ...
  const artistPromise = new Promise<GenreDb>((resolve, reject) => {
    ...
    async function fetchArtistsAsync(
      code: string,
...
export function getSelectedTracks(artists: Artist[], tracks: TrackItem[]) {
  const selectedArtists = artists.filter(
    (artist) => artist.selected && !artist.userDisabled
  );

  return tracks.filter((trackItem) => {
    return trackItem.track.artists.some(
      (trackArtist) =>
        selectedArtists.findIndex((artist) => artist.id === trackArtist.id) !==
        -1
    );
  });
}

...
const Playlist: React.FC<Props> = ({ tracks, createHandler, loaded }) => {
  ...
  function toggleTrackHandler(trackId: string) {
    if (disabledTrackIds.includes(trackId)) {
      setDisabledTrackIds(
        disabledTrackIds.filter((disabledId) => disabledId !== trackId)
      );
    } else {
  ...
  async function submitHandler(e: React.FormEvent<HTMLFormElement>) {
    ...
    const trackURIs = tracks
      .filter(
        (track) =>
          !disabledTrackIds.some((disabledId) => disabledId === track.track.id)
      )
  ...
  return (
    <div className={'playlist-container' + (loaded ? ' loaded' : '')}>
      <div className="loading-screen">
        ...
        <p className={'loading-text' + (loaded ? ' loaded' : '')}>
          Loading your tracks...
        </p>
      </div>
      <form
      ...
      >
        <input
          type="text"
          className={'playlist-title' + (loaded ? ' loaded' : '')}
          placeholder="Playlist Name"
          value={nameField}
          onChange={(e) => setNameField(e.target.value)}
        />
        {savingPlaylist ? (
          <div className="save-loader" data-testid={'playlist_test_loader'}>
            <CircleSpinner
              size={15}
              loading={loaded}
          ...
          <button
            className={'button-create' + (loaded ? ' loaded' : '')}
      ...
      {loaded && (
        <div data-testid={'playlist_test_div'} className="table-wrapper">
            ...
            <tbody className="track-list">
              {tracks.map((track) => (
                <PlaylistItem
                  track={track}
                  key={track.track.id}
                  disabled={disabledTrackIds.includes(track.track.id)}
                  toggleHandler={toggleTrackHandler}
                />
              ))}
            </tbody>
          </table>
